...
if (strlen($title) > 50 && strlen($content) > 400) {
    sendResponse(false, '标题字数小于50，内容字数大于1000');
}
$userId = $plaintext;
...
try {
    ...
    $updateAnnouncementSql = "UPDATE mok_group_announcement SET title = ?, content = ? WHERE id = ?";
    $stmt = $conn->prepare($updateAnnouncementSql);
    $stmt->bind_param("ssi", $title, $content, $announcementId);
    $stmt->execute();
    if ($stmt->affected_rows == 0) {
        $conn->rollback();
        sendResponse(false, '更新失败');
    }
    $logSql = "INSERT INTO mok_group_log (group_id, user_id, `action`, action_time, remark, ip_address, user_agent) 
               VALUES (?, ?, ?, NOW(), ?, ?, ?)";
    $remark = '修改公告ID:' . $announcementId;
