...
try {
    ...
    if ($action == 'pin') {
        if ($announcementInfo['is_top'] == 1) {
            $conn->rollback();
            sendResponse(false, '该公告已经置顶');
        }
        $updateSql = "UPDATE mok_group_announcement SET is_top = 1, update_time = NOW() WHERE id = ?";
        $stmt = $conn->prepare($updateSql);
        $stmt->bind_param("i", $announcementId);

        if (!$stmt->execute()) {
            $conn->rollback();
            sendResponse(false, '置顶操作失败：' . $stmt->error);
        }

        $message = '公告置顶成功';
    } else if ($action == 'unpin') {
        if ($announcementInfo['is_top'] == 0) {
            $conn->rollback();
            sendResponse(false, '该公告未置顶');
        }
        $updateSql = "UPDATE mok_group_announcement SET is_top = 0, update_time = NOW() WHERE id = ?";
        $stmt = $conn->prepare($updateSql);
        $stmt->bind_param("i", $announcementId);

        if (!$stmt->execute()) {
            $conn->rollback();
            sendResponse(false, '取消置顶操作失败：' . $stmt->error);
        }
    ...
    $logSql = "INSERT INTO mok_group_log (group_id, user_id, `action`, action_time, remark, ip_address, user_agent) 
               VALUES (?, ?, ?, NOW(), ?, ?, ?)";
    $remark = $action == 'pin' ? '置顶公告 ID: ' . $announcementId : '取消置顶公告 ID: ' . $announcementId;
    ...
    sendResponse(true, $message, [
        ...
        'is_top' => $updatedAnnouncement['is_top'],
        'title' => $updatedAnnouncement['title'],
        'update_time' => $updatedAnnouncement['update_time']
    ]);
} catch (Exception $e) {
    $conn->rollback();
    error_log("Group announcement pin/unpin error: " . $e->getMessage());
