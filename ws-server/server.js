...
const RELATION_API = process.env.PHP_RELATION_API;
...
const VIDEO_PORT = process.env.VIDEO_PORT || 8081;
const OFFLINE_MSG_API = '/offline_message/';
const SCHEDULED_MSG_API = '/scheduled_message/';
...
async function requestPHPAPI(path, params = {}, method = 'GET', body = null) {
  try {
    let url = `${PHP_API_BASE}${path}`;
...
async function loadScheduledMessages() {
  try {
    const schedules = await requestPHPAPI(SCHEDULED_MSG_API, { action: 'load' });
    ...
    for (const schedule of schedules) {
      if (schedule.repeatType === 'none' && schedule.scheduleTime < Date.now()) {
        console.log(`发现已过期的定时消息 [${schedule.scheduleId}]，计划时间: ${new Date(schedule.scheduleTime).toLocaleString()}`);
        expiredCount++;
        await requestPHPAPI(SCHEDULED_MSG_API, { action: 'fail' }, 'POST', {
...
async function saveScheduledMessage(schedule) {
  try {
    await requestPHPAPI(SCHEDULED_MSG_API, { action: 'update' }, 'POST', {
      scheduleId: schedule.scheduleId,
      nextScheduleTime: schedule.scheduleTime,
      sendCount: schedule.sendCount || 0
    });
    return true;
  } catch (error) {
    console.error(`保存定时消息失败 [${schedule.scheduleId}]:`, error.message);
    throw error;
  }
}
...
async function addScheduledMessage(scheduleData) {
  ...
  try {
    await requestPHPAPI(SCHEDULED_MSG_API, { action: 'add' }, 'POST', {
      scheduleId,
      senderId,
    ...
    console.log(`已添加定时消息 [${scheduleId}]，发送时间: ${new Date(scheduleTime).toLocaleString()}`);
    return schedule;
  } catch (error) {
    console.error(`添加定时消息失败:`, error.message);
    throw error;
  }
}
...
async function cancelScheduledMessage(scheduleId, userId) {
  ...
  try {
    await requestPHPAPI(SCHEDULED_MSG_API, { action: 'cancel', userId }, 'POST', { scheduleId });
    scheduledMessages.delete(scheduleId);
    console.log(`已取消定时消息 [${scheduleId}]`);
    return { success: true, message: '定时消息已取消' };
  } catch (error) {
    console.error(`取消定时消息失败 [${scheduleId}]:`, error.message);
...
async function getUserScheduledMessages(userId) {
  try {
    const schedules = await requestPHPAPI(SCHEDULED_MSG_API, { action: 'get', userId });
    return schedules;
  } catch (error) {
    console.error(`获取用户定时消息失败 [${userId}]:`, error.message);
    return [];
  }
}
...
async function executePrivateScheduledMessage(schedule) {
  ...
  if (!relationCheck.isValid) {
    ...
    try {
      await requestPHPAPI(SCHEDULED_MSG_API, { action: 'fail' }, 'POST', {
        scheduleId,
        errorMsg: relationCheck.tip
      });
    } catch (err) {
      console.error(`标记定时消息失败 [${scheduleId}]:`, err.message);
    }
    scheduledMessages.delete(scheduleId);
    return false;
  }
  ...
  if (schedule.repeatType !== 'none') {
    ...
    if (nextTime) {
      ...
      try {
        await requestPHPAPI(SCHEDULED_MSG_API, { action: 'complete' }, 'POST', { scheduleId });
      } catch (err) {
        console.error(`标记定时消息完成失败 [${scheduleId}]:`, err.message);
      }
      scheduledMessages.delete(scheduleId);
      return true;
    }
...
async function executeGroupScheduledMessage(schedule) {
  ...
  if (content && content.chain_hash) {
    forwardData.chain_prev_hash = content.chain_prev_hash;
    forwardData.chain_hash = content.chain_hash;
  }
...
async function saveOfflineMessage(userId, message) {
  try {
    const payload = {
      ...
      messageId: message.messageId || `msg_${Date.now()}_${Math.random().toString(36).substr(2, 8)}`,
      messageType: message.messageType || 'text',
      content: message.content,
      isGroup: message.isGroupMessage ? 1 : 0,
      isSystem: message.isSystem ? 1 : 0,
      isScheduled: message.isScheduled ? 1 : 0,
      scheduleId: message.scheduleId || null,
      sendTime: message.sendTime || Date.now(),
      lock: message.lock || null,
      status: 1
    };
    await requestPHPAPI(OFFLINE_MSG_API, { action: 'save' }, 'POST', payload);
    console.log(`已保存离线消息：用户 [${userId}] 来自 [${message.senderId}]`);
    return true;
  } catch (error) {
    console.error(`保存离线消息失败 [${userId}]:`, error.message);
    throw error;
  }
}
...
async function sendOfflineMessages(userId, ws) {
  try {
    const messages = await requestPHPAPI(OFFLINE_MSG_API, { action: 'get', userId });
    ...
    if (ws.readyState === WebSocket.OPEN) {
      ws.send(buildMessage('offline_messages_start', {
        ...
        message: `开始同步 ${messages.length} 条离线消息`
      }));
    }
    ...
    const sendNextMessage = async () => {
      if (messageQueue.length === 0) {
        ...
        if (failedMessages.length > 0) {
          console.log(`用户 [${userId}] 有 ${failedMessages.length} 条离线消息发送失败`);
        }

        setTimeout(async () => {
          try {
            await requestPHPAPI(OFFLINE_MSG_API, { action: 'delete', userId }, 'POST', {
              messageIds: messages.map(m => m.messageId)
            });
            console.log(`已删除用户 [${userId}] 的离线消息`);
          } catch (err) {
            console.error(`删除离线消息失败:`, err.message);
          }
        }, 5000);
      ...
      try {
        ...
        console.log(`发送离线消息 ${sentCount + 1}/${messages.length}: ${msg.messageId}`);
        sentCount++;
        const delay = messages.length > 100 ? 200 : 300;
    ...
    console.error(`获取离线消息失败 [${userId}]:`, error.message);
    if (ws.readyState === WebSocket.OPEN) {
      ws.send(buildMessage('offline_messages_complete', {
        totalCount: 0,
        error: error.message,
...
setInterval(() => {
  requestPHPAPI(OFFLINE_MSG_API, { action: 'cleanup' }).catch(() => {});
}, 24 * 60 * 60 * 1000);
